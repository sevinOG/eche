...
FIELD_HELP: dict[str, tuple[str, str]] = {
    ...
    "inf_api_key": (
        ...
        "don’t need a fancy GPU at home. Eche sends a question to the provider; the "
        "provider’s model writes the answer; Eche posts it in Discord.\n\n"
        "### Default: Groq (free tier)\n"
        "Eche is set up for **Groq** by default because it is fast and has a free tier:\n"
...
class SettingsWindow(QWidget):
    ...
    def _page_security(self) -> QWidget:
        ...
        c_hint = QLabel(
            ...
            "Eche.exe build, run a rebuild/update so the packaged app can see new files "
    ...
    def _page_updates(self) -> QWidget:
        ...
        self.check_btn.setToolTip("Runs eche_source\\BUILD.bat (one tap)")
        ...
        layout.addWidget(self._card(
            ...
            "(Eche.exe, _internal, assets\\icon.png).",
            body,
        ))
        layout.addStretch(1)
        return page
    ...
    def check_for_updates(self):
        """One-tap: run eche_source\\BUILD.bat (never the portable eche\\ tree)."""
        ...
        # Always score candidates: typed path may be polluted portable eche\
        ...
        if not project_path or not is_buildable_source(project_path):
            show_error(
                ...
                "In-app rebuild needs the **eche_source** folder with:\n"
                "• BUILD.bat + build_exe.spec\n"
                "• core/ and gui/\n"
                "• .venv (run once: python -m venv .venv && pip install -r requirements.txt)\n\n"
                "The portable **eche/** app folder is not enough — even if it has "
