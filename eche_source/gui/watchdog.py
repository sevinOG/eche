...
def is_gui_process(pid: int) -> bool:
    """Return True if PID is a running Eche GUI process (not the --bot child)."""
    try:
        if pid == os.getpid():
            return True
        ...
        # Source launches
        if "-m gui.main" in cmd or "run_gui.py" in cmd or "eche_app.py" in cmd:
            if "--bot" in cmd:
                return False
            return True

        # Frozen GUI (exe without --bot)
        if name in ("eche.exe", "eche_app.exe"):
...
def ensure_single_gui_instance():
    ...
    if "--bot" in sys.argv or os.environ.get("ECHE_RUNNING", "").upper() == "BOT":
        return True
