...
MANIFEST_NAME = ".echelon_install_manifest.json"
...
class InstallOptions:
    ...
    github_subdir: str = "echelon_source"
...
class Installer:
    ...
    def install(self, opts: InstallOptions) -> bool:
        try:
            ...
            self.log(f"[Echelon] Starting install v{APP_VERSION}")
            ...
            if opts.source_type == "github":
                ...
                if source_path.is_file() and (
                    source_path.parent.name in ("dist", "Echelon")
            ...
            if (
                ...
                shortcut_path = desktop / "Echelon.lnk"
                if create_shortcut(
                    str(main_exe),
                    str(shortcut_path),
                    str(install_path),
                    "Echelon - Elite Operations",
                    brand_icon,
                ):
            ...
            if (
                ...
                echelon_menu = start_menu / "Echelon"
                echelon_menu.mkdir(parents=True, exist_ok=True)
                shortcut_path = echelon_menu / "Echelon.lnk"
                if create_shortcut(
                    str(main_exe),
                    str(shortcut_path),
                    str(install_path),
            ...
            try:
                ...
                if getattr(sys, 'frozen', False):
                    ...
                    if opts.create_start_menu_shortcut:
                        sm_uninstall = get_start_menu_path() / "Echelon" / "Uninstall Echelon.lnk"
                        create_shortcut(str(uninstaller_path), str(sm_uninstall), str(install_path), "Uninstall Echelon", str(uninstaller_path))
            ...
            self.log(f"[SUCCESS] Echelon installed to {install_path}")
            if main_exe and Path(main_exe).exists():
                self.log(f"Launch: {main_exe}")
                # Persist launch path for the Finish-page button
                try:
                    launch_marker = install_path / ".echelon_launch_path"
                    launch_marker.write_text(str(main_exe), encoding="utf-8")
                except Exception:
                    pass
            else:
                self.log("WARNING: Could not locate Echelon.exe after copy — Launch may fail")
    ...
    def _recover_source_from_app(
        ...
        if source_path.is_file():
            ...
            if (source_path / "Echelon.exe").is_file():
                roots.append(source_path)
                roots.append(source_path / "_internal")
        ...
        if not (dest / "echelon_app.py").is_file():
            (dest / "echelon_app.py").write_text(
            ...
            copied_files.append(str(dest / "echelon_app.py"))
    ...
    def _find_main_exe(self, install_path: Path):
        ...
        prefer = [
            ...
            install_path / "Echelon" / "Echelon.exe",
        ...
        for p in sorted(install_path.glob("*.exe")):
            if p.is_file() and "uninstall" not in p.name.lower():
                return p
        for p in sorted(install_path.rglob("Echelon.exe")):
            if p.is_file():
                return p
...
def main():
    ...
    manifest = install_dir / ".echelon_install_manifest.json"
    print(f"Uninstalling Echelon from {install_dir}")
    if manifest.exists():
        try:
    ...
    try:
        import winreg
        winreg.DeleteKey(winreg.HKEY_CURRENT_USER, r"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Echelon")
    except Exception:
        pass
