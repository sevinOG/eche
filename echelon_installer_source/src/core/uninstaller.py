...
class Uninstaller:
    ...
    def uninstall(self, install_dir: str, remove_dir: bool = True) -> bool:
        try:
            ...
            self.log(f"[Echelon] Uninstalling from {install_path}")
            ...
            common_shortcuts = [
                Path(os.path.expanduser("~/Desktop/Echelon.lnk")),
                Path(os.path.expanduser("~/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Echelon/Echelon.lnk")),
                Path(os.path.expanduser("~/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Echelon/Uninstall Echelon.lnk")),
            ]
            ...
            self.log("[SUCCESS] Echelon uninstalled")
            return True
    ...
    def _schedule_self_delete(self, install_path: Path):
        try:
            import tempfile
            bat_path = Path(tempfile.gettempdir()) / f"echelon_uninstall_{int(time.time())}.bat"
