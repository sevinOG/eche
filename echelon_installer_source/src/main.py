...
from src.ui.installer_window import EchelonInstallerWindow
from src.ui.theme import VERSION

def handle_cli_uninstall():
    """If run with --uninstall, do silent uninstall or GUI uninstall mode"""
    parser = argparse.ArgumentParser(description=f"Echelon Installer v{VERSION}")
    parser.add_argument("--uninstall", action="store_true", help="Uninstall Echelon")
    ...
    if args.uninstall:
        ...
        if args.silent:
            ...
            print(f"[Echelon] Silent uninstall from {install_dir}")
            u = Uninstaller()
...
def main():
    ...
    app.setApplicationName("Echelon Installer")
    app.setApplicationVersion(VERSION)
    app.setStyle("Fusion")

    # Same brand as Echelon: assets/icon.png (bundled in frozen builds)
    try:
