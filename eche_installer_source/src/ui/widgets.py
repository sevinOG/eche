"""Custom widgets for eche installer"""
from PyQt6.QtWidgets import QWidget, QHBoxLayout, QVBoxLayout, QLabel, QFrame
from PyQt6.QtCore import Qt
from .theme import ECHE_PALETTE
...
class StepIndicator(QWidget):
    ...
    def _build(self):
        for i, name in enumerate(self.steps):
            ...
            if i > 0:
                ...
                sep.setStyleSheet(f"background-color: {ECHE_PALETTE['border']};")
    ...
    def set_current(self, idx: int):
        ...
        for i, (dot, num) in enumerate(self.dots):
            lbl = self.labels[i]
            if i < idx:
                bg = ECHE_PALETTE["success"]
                num.setText("✓")
                num.setStyleSheet(
                    "background: transparent; color: white; font-size: 11px; font-weight: 700;"
                )
                lbl.setStyleSheet(
                    f"background: transparent; color: {ECHE_PALETTE['success']}; font-size: 11px;"
                )
                dot.setStyleSheet(
                    f"background-color: {bg}; border-radius: 14px; border: none;"
                )
            elif i == idx:
                bg = ECHE_PALETTE["accent"]
                num.setText(str(i + 1))
                num.setStyleSheet(
                    "background: transparent; color: white; font-size: 11px; font-weight: 700;"
                )
                lbl.setStyleSheet(
                    f"background: transparent; color: {ECHE_PALETTE['text']}; "
                ...
                lbl.setStyleSheet(
                    f"background: transparent; color: {ECHE_PALETTE['text_dim']}; font-size: 11px;"
                )
                dot.setStyleSheet(
                    f"background-color: {ECHE_PALETTE['surface_2']}; "
                    f"border: 1px solid {ECHE_PALETTE['border']}; border-radius: 14px;"
                )
        self.update()
...
class Card(QFrame):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            f"#Card {{ background-color: {ECHE_PALETTE['surface']}; "
            f"border: 1px solid {ECHE_PALETTE['border']}; border-radius: 12px; }}"
        )
