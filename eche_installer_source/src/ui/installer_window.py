"""Main installer GUI window - Eche themed wizard (spacious layout)."""
import os
import sys
import platform
...
from .theme import STYLESHEET, ECHE_PALETTE, VERSION
...
class EcheInstallerWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Eche Installer v{VERSION}")
        ...
        self.source_path, self.source_type = find_eche_source()
        ...
        # Dev source tree (eche_source) vs portable app (eche)
        src = find_source_tree()
        self.default_repo = src if src is not None else (workspace_root() / "eche_source")
    ...
    def _build_ui(self):
        ...
        header.setStyleSheet(
            f"background-color: {ECHE_PALETTE['surface']}; "
            f"border-bottom: 1px solid {ECHE_PALETTE['border']};"
        )
        ...
        glow.setStyleSheet(f"background-color: {ECHE_PALETTE['accent']};")
        main_layout.addWidget(glow)

        # --- Step strip (own row so header is not cramped) ---
        step_bar = QFrame()
        step_bar.setStyleSheet(
            f"background-color: {ECHE_PALETTE['bg_alt']}; "
            f"border-bottom: 1px solid {ECHE_PALETTE['border']};"
        )
        ...
        nav.setStyleSheet(
            f"background-color: {ECHE_PALETTE['surface']}; "
            f"border-top: 1px solid {ECHE_PALETTE['border']};"
        )
    ...
    def _build_welcome_page(self):
        ...
        brand.setStyleSheet(
            ...
            f"letter-spacing: 8px; color: {ECHE_PALETTE['accent']};"
        )
        ...
        tag.setStyleSheet(
            ...
            f"color: {ECHE_PALETTE['text_muted']};"
        )
        ...
        desc = QLabel(
            ...
            "  3) Open START_HERE.txt — double-click Eche.exe if present,\n"
        ...
        desc.setStyleSheet(
            f"background: transparent; color: {ECHE_PALETTE['text_muted']}; "
            f"font-size: 13px;"
        )
        ...
        src_title.setStyleSheet(
            f"background: transparent; color: {ECHE_PALETTE['accent']}; font-weight: 700;"
        )
        ...
        if self.existing_installed and self.existing_dir:
            ...
            reinstall.setStyleSheet(
                f"background: {ECHE_PALETTE['surface_2']}; "
                f"border: 1px solid {ECHE_PALETTE['accent']}; border-radius: 6px; "
                f"padding: 10px; color: {ECHE_PALETTE['text_muted']}; font-size: 11px;"
            )
        ...
        footer.setStyleSheet(
            f"background: transparent; color: {ECHE_PALETTE['text_dim']}; "
    ...
    def _build_location_page(self):
        ...
        head.setStyleSheet(
            f"background: transparent; font-size: 18px; color: {ECHE_PALETTE['text']}; "
            f"font-weight: 700;"
        )
        ...
        info = QLabel(
            ...
            "App install → usually the portable eche/ folder or LocalAppData.\n"
            "Source recovery → usually eche_source/ (or any empty folder)."
        )
        ...
        src_head.setStyleSheet(
            f"background: transparent; font-size: 15px; color: {ECHE_PALETTE['text']}; "
            f"font-weight: 700;"
        )
        ...
        self.radio_github = QRadioButton(
            "Install from GitHub (recommended) — eche_source"
        )
        ...
        self.radio_exe = QRadioButton("Install APP from portable / Eche.exe")
        self.radio_exe.toggled.connect(self._on_source_mode_toggled)
        adv_l.addWidget(self.radio_exe)

        self.radio_source = QRadioButton("Install APP from local eche_source tree")
        ...
        self.repo_hint.setStyleSheet(
            f"font-size: 11px; color: {ECHE_PALETTE['text_muted']}; "
    ...
    def _build_options_page(self):
        ...
        for text, attr, default in (
            ...
            ("Launch Eche after installation", "chk_launch", True),
        ):
            cb = QCheckBox(text)
    ...
    def _build_install_page(self):
        ...
        self.btn_install_now = QPushButton("INSTALL ECHE")
    ...
    def _build_finish_page(self):
        ...
        self.finish_icon.setStyleSheet(
            f"background: {ECHE_PALETTE['success']}; color: white; font-size: 22px; "
        ...
        self.finish_title.setStyleSheet(
            f"background: transparent; font-size: 22px; font-weight: 800; "
            f"color: {ECHE_PALETTE['text']};"
        )
        layout.addWidget(self.finish_title)

        self.finish_desc = QLabel(f"Eche has been successfully deployed to\n{self.install_dir}")
        ...
        self.finish_desc.setStyleSheet(
            f"background: transparent; color: {ECHE_PALETTE['text_muted']}; font-size: 13px;"
        )
        ...
        self.btn_launch = QPushButton("Launch Eche")
    ...
    def _start_install(self):
        ...
        if getattr(self, "radio_github", None) and self.radio_github.isChecked():
            ...
            if src_path.is_file() and src_path.suffix.lower() == ".exe":
                ...
                if (src_path / "Eche.exe").is_file() or (src_path / "_internal").is_dir():
                    stype = "portable_dir" if (src_path / "Eche.exe").is_file() else "dist_dir"
                    if (src_path / "Eche.exe").is_file():
                        # install from onedir parent of exe
                        source_text = str(src_path / "Eche.exe")
                        stype = "exe"
    ...
    def _on_uninstall_clicked(self):
        ...
        reply = QMessageBox.question(
            self,
            "Uninstall Eche",
            f"Uninstall Eche from {self.existing_dir}?",
    ...
    def _on_uninstall_finished(self, success: bool):
        if success:
            ...
            self.finish_desc.setText(f"Eche has been removed from {self.existing_dir}")
            self.finish_icon.setText("X")
            self.finish_icon.setStyleSheet(
                f"background: {ECHE_PALETTE['text_muted']}; color: white; "
