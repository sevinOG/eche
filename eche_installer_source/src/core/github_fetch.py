"""Download Eche packages from the public GitHub hub (no Git account required)."""
...
USER_AGENT = "Eche-Installer/1.3 (+https://github.com/sevinOG/echelon_ecosystem)"
...
def fetch_source_from_github(
    ...
    *,
    ...
    progress(35, "Extracting Eche source…")
    ...
    if not (dest / "core").is_dir() and not (dest / "eche_app.py").is_file():
    ...
    if not (dest / "core").is_dir():
        raise RuntimeError(
            "Downloaded archive did not contain eche_source/core. "
...
def try_fetch_portable_app_from_release(
    ...
    *,
    ...
    Returns path to Eche.exe if found, else None (beginners can still use source).
    """
    log = log or (lambda _m: None)
    progress = progress or (lambda _p, _m: None)
    dest = Path(dest_dir)
    ...
    # Prefer zip named portable / Eche, then onedir-ish exe
    pick = None
    for a in assets:
    ...
    if pick is None:
        for a in assets:
            name = (a.get("name") or "").lower()
            if name.endswith(".zip") and "eche" in name and "installer" not in name:
                pick = a
                break
    if pick is None:
...
def fetch_to_temp_source(
    *,
    ...
    tmp = Path(tempfile.mkdtemp(prefix="eche_gh_src_"))
