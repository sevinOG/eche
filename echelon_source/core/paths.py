...
# Three-tree layout:
#
#   echelon/              ← portable app (Echelon.exe + data) — flash-drive ready
#   echelon_source/       ← this tree when developing (core/ gui/ cogs/)
#   echelon_installer/    ← deploy / recover wizard
#
# Writable data always lives next to the package root (portable), never AppData
# unless the user overrides ECHELON_USER_ROOT.
...
def _looks_like_portable_app(path: str | None) -> bool:
    """Runnable app folder: Echelon.exe co-located with data dirs."""
    if not path or not os.path.isdir(path):
        return False
    if os.path.isfile(os.path.join(path, "Echelon.exe")):
        return True
    # onedir nested under dist/Echelon
    nested = os.path.join(path, "Echelon.exe")
...
def is_buildable_source(path: str | None) -> bool:
    ...
    if os.path.isfile(os.path.join(path, "Echelon.exe")) and not has_build_venv(path):
        return False
    return True
...
def _read_install_json(base: str) -> dict:
    for path in (
        ...
        os.path.join(base, "dist", "Echelon", "install.json"),
    ):
...
def package_root() -> str:
    ...
    - Frozen portable: folder containing Echelon.exe (flash-drive root)
    - Source: echelon_source/ (parent of core/)
    """
    if is_frozen():
        exe_dir = os.path.dirname(os.path.abspath(sys.executable))
        # Prefer the directory that holds the exe (true portable root)
        if os.path.isfile(os.path.join(exe_dir, "Echelon.exe")) or os.path.basename(
            sys.executable
        ).lower().startswith("echelon"):
            ...
            if found and _looks_like_source_tree(found):
                # Running from source dist/Echelon — package root is source tree
                return found
            return exe_dir
    ...
    # Source: core/paths.py → echelon_source root
...
def user_dir() -> str:
    ...
    Override with ECHELON_USER_ROOT only for a deliberate split layout.
    """
    override = (os.environ.get("ECHELON_USER_ROOT") or "").strip()
...
def dist_exe() -> str | None:
    """Path to a built Echelon.exe if present under source or portable tree."""
    root = package_root()
    candidates = [
        os.path.join(root, "Echelon.exe"),
        os.path.join(root, "dist", "Echelon", "Echelon.exe"),
        os.path.join(root, "dist", "Echelon.exe"),
    ]
    # sibling portable tree when running from source
    sibling = os.path.join(os.path.dirname(root), "echelon", "Echelon.exe")
...
def _score_source_candidate(path: str) -> int:
    ...
    if base == "echelon_source":
        score += 50
    elif base in ("echelon-source",):
        score += 40
    elif base == "echelon":
...
def source_root(stored: str | None = None) -> str | None:
    """
    Locate a *buildable* echelon_source tree for one-tap rebuilds.
    Never returns a portable install folder that only looks like source
    (installer may have copied core/ into echelon/).
    """
    candidates: list[str] = []
    ...
    env_src = (os.environ.get("ECHELON_SOURCE_ROOT") or "").strip()
    if env_src:
        _add(env_src)

    root = package_root()
    parent = os.path.dirname(root)
    # Always prefer sibling echelon_source next to portable app / workspace
    for name in ("echelon_source", "echelon-source"):
        _add(os.path.join(parent, name))
        _add(os.path.join(root, name))
        _add(os.path.join(os.path.dirname(parent), name))

    _add(root)
    # Workspace-style: …/workspace/{echelon,echelon_source,echelon_installer}
    for name in ("echelon_source", "echelon-source"):
        _add(os.path.join(parent, name))
...
def _looks_like_portable_only(path: str | None) -> bool:
    ...
    if is_buildable_source(path) and has_build_venv(path):
        ...
        if os.path.basename(os.path.abspath(path)).lower() == "echelon_source":
            return False
    has_exe = os.path.isfile(os.path.join(path, "Echelon.exe"))
...
def resolve_source_root(stored: str | None = None) -> str:
    ...
    if found:
        return found
    # Do not fall back to portable package_root — that caused BUILD in echelon/
    return found or ""


def build_script_path(stored: str | None = None) -> str | None:
    """Path to echelon_source/BUILD.bat if available."""
