...
def is_enabled() -> bool:
    import os
    return (
        os.environ.get("ECHELON_RUNNING", "").upper() == "BOT"
        or os.environ.get("ECHELON_GUI_BRIDGE", "").strip() == "1"
    )
...
def normalize_cog_module(module: str) -> str:
    ...
    if module.startswith("echelon_ecosystem."):
        module = module[len("echelon_ecosystem.") :]
