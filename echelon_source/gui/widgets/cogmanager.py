...
Drop any `.py` file onto the drop zone. Echelon inspects the source and
...
Frozen builds write to the install folder’s `cogs/` (next to Echelon.exe), not inside `_internal`.
"""
...
class CogManagerWindow(QMainWindow):
    ...
    def _short_module(ext: str) -> str:
        ext = (ext or "").strip()
        if ext.startswith("echelon_ecosystem."):
            return ext[len("echelon_ecosystem.") :]
        return ext
    ...
    def _on_toggle(self, display_module, checked):
        ...
        if display_module.startswith(("cogs.", "core.")):
            full = display_module
        elif display_module.startswith("echelon_ecosystem."):
            full = display_module[len("echelon_ecosystem.") :]
        else:
